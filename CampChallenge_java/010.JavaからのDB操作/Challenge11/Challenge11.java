...
public class Challenge11 extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            //DBのデータを更新(IDが存在しない場合の処理は未記入)
            db_st = db_con.prepareStatement("update profiles set name=?, tell=?,"
                    + "age=?, birthday=? where profilesID=?");
            db_st.setString(1, in_name);
            db_st.setString(2, in_tell);
            db_st.setInt(3, in_age);
            db_st.setString(4, in_birthday);
            db_st.setInt(5, in_profilesID);
